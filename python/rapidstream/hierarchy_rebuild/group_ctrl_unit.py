...
def embed_ctrl_unit(config: Dict, target_vertex: str, ctrl_vertex: str) -> Dict:
  ...
  # send out constants
  # FIXME: do not support sending constant arguments to other islands
  # ctrl_wrapper_props['port_wire_map']['constant_out'] = ctrl_props['port_wire_map']['constant_ports']
  for portname, argname in ctrl_props['port_wire_map']['constant_ports'].items():
    _logger.critical('temp hack to remove constant wire %s from top rtl', argname)
    config['wire_decl'].pop(argname)
